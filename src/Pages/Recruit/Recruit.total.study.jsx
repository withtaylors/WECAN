...
function RecruitStudyCards() {
  ...
  ///////////////////////////////////////////////
  const studyChallenge = challengeArray.filter(
    (challenge) => challenge.category === 'study'
  );
  ...
  const currentCategories = studyChallenge.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(studyChallenge.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
...
export default RecruitStudyCards;
